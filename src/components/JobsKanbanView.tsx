...
export const JobsKanbanView: React.FC<JobsKanbanViewProps> = ({ 
  ...
  const handleDropColumn = async ( 
    ...
    const newOrdem = targetColumnJobs.length + 1;

    setJobs((prev) => 
      prev.map((j) => (j.id === jobId ? { ...j, status: targetStatus, ordem: newOrdem } : j)), 
    ); 
    ...
    try { 
      ...
      if (data.sucesso) { 
        if (onSaveJob) { 
          await onSaveJob({ id: targetJob.id, status: targetStatus, ordem: newOrdem }); 
        } 
