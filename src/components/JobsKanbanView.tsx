...
export const JobsKanbanView: React.FC<JobsKanbanViewProps> = ({
  ...
  const filteredJobs = jobs?.filter((j) => {
    const isArchived =
      (j as any).arquivado === 1 || (j as any).arquivado === "1";
  ...
  const updateJobOrdersInBackend = async (updatedColumnJobs: Job[]) => {
    try {
      await Promise.all(
        updatedColumnJobs.map((job, index) => {
          const formData = new FormData();
          formData.append("tabela", "jobs");
          formData.append("id", job.id);
          formData.append("ordem", String(index + 1));
          return fetch("https://sothink.com.br/app/api/editar", {
            method: "POST",
            body: formData,
          });
        }),
  ...
  const handleDragOverCard = (e: React.DragEvent, targetJobId: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (draggedJobId !== targetJobId) {
      setDragOverJobId(targetJobId);
    }
  };
  ...
  const handleDropCard = async (e: React.DragEvent, targetJob: Job) => {
    e.preventDefault();
    e.stopPropagation();
    setDragOverJobId(null);

    const sourceJobId = e.dataTransfer.getData("text/plain") || draggedJobId;
    if (!sourceJobId || sourceJobId === targetJob.id) return;

    const sourceJob = jobs.find((j) => j.id === sourceJobId);
    if (!sourceJob) return;

    // Obtém jobs da coluna alvo ordenados
    const columnJobs = getSortedColumnJobs(targetJob.status);
    const sourceIndex = columnJobs.findIndex((j) => j.id === sourceJobId);
    const targetIndex = columnJobs.findIndex((j) => j.id === targetJob.id);

    let updatedList = [...columnJobs];
    ...
    if (sourceIndex !== -1) {
      ...
      // Movendo de outra coluna e inserindo na posição específica
      const updatedSourceJob = { ...sourceJob, status: targetJob.status };
      updatedList.splice(targetIndex, 0, updatedSourceJob);
    }

    // Atribui novo índice sequencial 'ordem'
    const reorderedColumnJobs = updatedList.map((j, idx) => ({
      ...j,
    ...
    await updateJobOrdersInBackend(reorderedColumnJobs);
  ...
  const handleDropColumn = async (
    ...
    const targetColumnJobs = getSortedColumnJobs(targetStatus);
    const newOrdem = 0;
    ...
    try {
      ...
      formData.append("ordem", String(newOrdem));
  ...
  // Ordenação inteligente: Respeita ordem manual e intercala novos jobs pela Data de Início
  const getSortedColumnJobs = (statusId: JobStatus) => {
    const rawColumnJobs =
      filteredJobs?.filter((j) => j.status === statusId) || [];

    // 1. Função para extrair a data de forma segura (joga pro final se não tiver data)
    const parseDate = (dateStr: any) => {
      if (!dateStr || dateStr.startsWith("0000")) return Infinity;
      return new Date(dateStr).getTime();
    };

    // 2. Separar os jobs em dois grupos: Os que TÊM ordem fixa vs Os NOVOS (sem ordem)
    const ordered = rawColumnJobs
      .filter((j) => {
        const num = Number((j as any).ordem);
        return !isNaN(num) && num > 0; // Só os que você já arrastou (1, 2, 3...)
      })
      .sort((a, b) => Number((a as any).ordem) - Number((b as any).ordem));

    const unordered = rawColumnJobs
      .filter((j) => {
        const num = Number((j as any).ordem);
        return (
          isNaN(num) ||
          num === 0 ||
          (j as any).ordem === null ||
          (j as any).ordem === undefined
        );
      })
      .sort((a, b) => parseDate(a.data_inicio) - parseDate(b.data_inicio)); // Organiza os novos entre si pela data

    // 3. A Mágica: Intercalar os jobs novos no meio da lista que já tem ordem fixa
    let merged = [...ordered];

    unordered.forEach((uJob) => {
      const uDate = parseDate(uJob.data_inicio);
      ...
      if (insertIndex === -1) {
        // Se não achou ninguém com data maior (ou se é infinito), ele vai pro fim da fila
        merged.push(uJob);
      } else {
        // Se achou, injeta o job novo exatamente nessa posição!
        merged.splice(insertIndex, 0, uJob);
      }
    });
      ...
      {viewMode === "kanban" && (
        <div className="flex gap-4 overflow-x-auto pb-6 pt-1 snap-x min-h-[600px]">
          {KANBAN_COLUMNS.map((col) => {
            const columnJobs = getSortedColumnJobs(col.id);
                ...
                <div className="flex-1 overflow-y-auto space-y-3 my-2 pr-1">
                  {columnJobs.length === 0 ? (
                    ...
                    columnJobs.map((job) => {
                      ...
                      const isDragOver = dragOverJobId === job.id;
                      ...
                      return (
                        <div
                          ...
                          onDragOver={(e) => handleDragOverCard(e, job.id)}
                          onDragLeave={handleDragLeaveCard}
                          onDrop={(e) => handleDropCard(e, job)}
                          onClick={() => setActiveJob(job)}
                          className={`p-4 bg-white dark:bg-slate-900 rounded-lg border shadow-xs hover:shadow-sm hover:border-blue-500/60 transition-all cursor-grab active:cursor-grabbing space-y-3 text-xs ${
                            isDragOver
                              ? "border-indigo-500 border-2 bg-indigo-50/20"
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="font-extrabold text-slate-900 dark:text-white text-base flex items-center gap-2">
                <CalendarIcon className="w-5 h-5 text-indigo-500" /> Calendário
                Mensal
              </h3>
          ...
          <div className="grid grid-cols-7 gap-px bg-slate-200 dark:bg-slate-700 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            {Array.from({ length: daysInMonth }).map((_, idx) => {
              ...
              return (
                <div
                  key={day}
                  className={`bg-white dark:bg-slate-900 p-1.5 min-h-[120px] transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50 flex flex-col ${
                    isToday ? "ring-2 ring-inset ring-indigo-500" : ""
                  }`}
                >
                  <span
                    className={`inline-block mb-1 text-xs font-bold w-6 h-6 text-center leading-6 rounded-full ${
                  ...
                  <div className="flex-1 space-y-1 overflow-y-auto max-h-[100px] pr-1">
                    {jobsForDay?.map((job) => (
                      <div
                        key={job.id}
                        onClick={() => setActiveJob(job)}
                        className="text-[10px] p-1.5 rounded-md bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800/60 cursor-pointer hover:border-indigo-300 overflow-hidden shadow-xs transition-colors flex justify-between items-start gap-1"
